...
int main()
{
    ...
    for (int i = 0; i < N_LINHA; i++)
    {
        ...
        instrucao[i].orgem = org;
        // fazendo as alocações das instruções conforme leitura
        if (strcmp(intrucao[i].instr_nome, vet_1) == 0)
        {
            instrucao[i].num_instrucao = 1;
            valor[i] = instrucao[i].orgem;
        }
        else if (strcmp(intrucao[i].instr_nome, vet_2) == 0)
        {
            instrucao[i].num_instrucao = 2;
        }
        else if (strcmp(intrucao[i].instr_nome, vet_3) == 0)
        {
            instrucao[i].num_instrucao = 3;
        }
        else if (strcmp(intrucao[i].instr_nome, vet_4) == 0)
        {
            instrucao[i].num_instrucao = 4;
        }
        else if (strcmp(intrucao[i].instr_nome, vet_5) == 0)
        {
            instrucao[i].num_instrucao = 5;
        }
        else if (strcmp(intrucao[i].instr_nome, vet_6) == 0)
        {
    ...
    for (int i = 0; i < N_LINHA && alert == 0; i++)
    {
